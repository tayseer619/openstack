...
class test_verify_create_sriov_instance_from_snapshot(BaseTestCase):
    """A Test case for create_sriov_instance_from_snapshot"""

    name = "create_sriov_instance_from_snapshot"
    ...
    def run_test(self, testbed_obj):
        ...
        if self.instance_snapshot_id != None:
            self.instance2 = common_utils.search_and_create_server(
                overcloud_ep, overcloud_token, settings["server_2_name"], self.instance_snapshot_id, 
                settings["key_name"], self.flavor_id,  common_utils.ids.get("network1_id"), common_utils.ids.get("security_group_id"))
