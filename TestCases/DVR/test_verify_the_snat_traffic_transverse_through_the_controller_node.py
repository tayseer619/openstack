...
class TestVerifyTheSnatTrafficTransverseThroughTheControllerNode(BaseTestCase):
    ...
    def pre_testcase(self, testbed_obj):
        ...
        self.instance = common_utils.create_instance(
            testbed_obj, self.flavor_id, settings["server_1_name"],
            settings["network1_name"], common_utils.ids.get("network1_id"),
            self.compute0)
    ...
    def run_test(self, testbed_obj):
        ...
        if ping_response1 == 0:
            router_namespace = "snat-"+common_utils.ids.get("router_id")
            received_icmp = utils.verify_traffic_on_namespace(
                self.controller0_ip, router_namespace,
