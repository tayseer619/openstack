...
class NumaInstanceAllVcpusConsumed(BaseTestCase):
    """A Test case to verify all vcpus are consumed"""
    name = "verify all vcpus are consumed"
    ...
    def pre_testcase(self, testbed_obj):
        ...
        self.flavor_id = common_utils.get_flavor_id(
            testbed_obj, "dpdk", settings.get("flavor1_name"), 20)
    ...
    def run_test(self, testbed_obj):
        ...
        instances_possible = utils2.get_possible_numa_instances(
            self.compute0_ip, 20)
        for i in range(0, instances_possible):
            self.instance = common_utils.create_instance(testbed_obj, self.flavor_id, 'server_name{}'.format(
                i), settings["network1_name"], common_utils.ids.get("network1_id"), self.compute0, "Yes")
