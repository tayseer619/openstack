...
class test_verify_communication_of_ovs_offload_and_simple_instance_on_different_compute_and_same_network(BaseTestCase):
    """A Basic Test Case for test_verify_communication_of_ovs_offload_and_simple_instance_on_different_compute_and_same_network"""

    name = "test_verify_communication_of_ovs_offload_and_simple_instance_on_different_compute_and_same_network"
    ...
    def pre_testcase(self, testbed_obj):
        ...
        self.compute1_ip = common_utils.get_compute_ip(
            testbed_obj, "compute-1")
        ...
        self.instance2 = common_utils.create_instance(
            testbed_obj, self.flavor_id, settings["server_2_name"], settings["network1_name"], common_utils.ids.get("network1_id"), self.compute1)
    ...
    def run_test(self, testbed_obj):
        ...
        if self.instance1.get("status") == "active" and self.instance2.get("status") == "active":
            received_icmp = utils.verify_offloading_on_representor_port(
                self.compute1_ip, self.instance1.get("floating_ip"), self.instance2.get("floating_ip"), settings)
