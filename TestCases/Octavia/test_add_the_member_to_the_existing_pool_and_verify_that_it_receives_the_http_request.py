...
class test_verify_the_working_of_loadbalancer_by_sending_http_traffic_to_loadbalancer(BaseTestCase):
    """A Test case for the_working_of_loadbalancer_by_sending_http_traffic_to_loadbalancer"""
    name = "working_of_loadbalancer_by_sending_http_traffic_to_loadbalancer"
    ...
    def run_test(self, testbed_obj):
        ...
        if self.loadbalancer.get("pool_status") == "ACTIVE":
            ...
            utils.add_members_to_pool(overcloud_ep, overcloud_token, self.loadbalancer.get(
                "pool_id"), common_utils.ids.get("subnet1_id"), 80, "HTTPS", [self.instance1])
            round_robin = utils.roundrobin_traffic_test(
                self.loadbalancer.get("floating_ip"), "HTTPS")

            utils.add_members_to_pool(overcloud_ep, overcloud_token, self.loadbalancer.get(
                "pool_id"), common_utils.ids.get("subnet1_id"), 80, "HTTPS", [self.instance2])
            round_robin = utils.roundrobin_traffic_test(
                self.loadbalancer.get("floating_ip"), "HTTPS")
