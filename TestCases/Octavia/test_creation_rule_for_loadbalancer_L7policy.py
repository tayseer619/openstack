...
class test_verify_creation_of_loadbalancer_L7policy(BaseTestCase):
    """A Test case for creation_of_loadbalancer_L7policy"""
    name = "creation_of_loadbalancer_L7policy"
    ...
    def run_test(self, testbed_obj):
        ...
        l7policy_id = utils.create_l7policy(
            overcloud_ep, overcloud_token, self.loadbalancer.get("listener_id"))
        l7_policy_rule = utils.create_l7policy_rule(
            overcloud_ep, overcloud_token, l7policy_id)
        assert l7policy_id != None
        assert l7_policy_rule != None
