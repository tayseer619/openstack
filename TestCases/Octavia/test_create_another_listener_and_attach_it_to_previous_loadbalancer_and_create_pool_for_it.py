...
class test_verify_the_creation_of_http_loadbalancer(BaseTestCase):
    ...
    def run_test(self, testbed_obj):
        ...
        listener2_state = ""
        try:
            listener2_id = utils.search_and_create_listener(overcloud_ep, overcloud_token, settings.get(
                "listener2_name"), self.loadbalancer.get("lb_id"), "HTTPS", 80)
            utils.listener_build_wait(overcloud_ep, overcloud_token, [
                                      listener2_id], settings.get("loadbalancer_listener_creation_retires"))
            listener2_state = utils.check_listener_status(
                overcloud_ep, overcloud_token, listener2_id)
        except:
